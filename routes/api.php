...
Route::post('/flightPrice', function (Request $request) {
    try {
        if(!$request->has(["id"])) {
            $data = ['estado' => 'erro', 'desc' => 'Submeta todos os campos'];
            return response()->json($data, 406, []);
...
Route::post('/flightBook', function (Request $request) {
    function getRandomPlaces($id_voo_tarifa)
    {
    ...
    try {
        if (!$request->has(["qtd", "tipo", "id_voo_tarifa"])) {
            $data = ['estado' => 'erro', 'desc' => 'Submeta todos os campos'];
            return response()->json($data, 406, []);
        }
        ...
        for ($i = 1; $i <= $request->qtd; $i++) {
            ...
            $idPlace = getRandomPlaces($request->id_voo_tarifa);
